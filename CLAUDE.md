...
- **`cleanupTimeout` only races, never cancels.** When `options.cleanupTimeout` is set, `markAsEnded` races `Promise.allSettled(...returns)` against a `setTimeout` — the pending `.return()` promises are *abandoned*, not cancelled (promises aren't cancellable). That's the trade-off for unblocking a wedged source. The default `undefined` keeps the unbounded `await` to match native `AsyncGenerator`. Don't make it default-to-a-number — silent behaviour shifts on close paths are how people get bitten.
