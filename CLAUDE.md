...
### The eager lane machine (`ordered: 'eager'`)

The third dispatch mode is fully isolated behind `mode === 'eager'` checks at construction and in `next()`; the two loops above are untouched (their `ordered`-truthiness sites became `mode === 'ordered'` renames). One `Lane<R>` per in-flight source item, in source order — `lanes[0]` is the delivery head; each lane holds a private `buffer` of ≤ `lookahead` values (`laneLookahead`, option-driven, default `DEFAULT_LANE_LOOKAHEAD` = 1), a never-rejecting `pending` step promise, and `dispatched`/`done`/`terminalErr`/`phantom` state.

- **`admitLanes()`/`admitOneLane()`** — the producer: admits placeholder lanes up to `bufferSize` (same stop-set as `fillQueue`, plus `mainReturnedDone`). The source's own FIFO `.next()` ordering binds lane order == source order. `lane.pending` stays set across the source pull AND the callback dispatch — clearing it mid-dispatch would leave a parked consumer racing `undefined` and busy-looping (that was an OOM). Both dispatch windows are `isDone`-guarded: a pull or callback settling after close must not spawn a sub-iterator nothing can reach (its `finally` would never run); one that slips through the second window is closed on the spot. Fan-out uses the shared `subIteratorFromResolved` — never a local copy.
- **`pumpLane(lane)`** — the background drainer: one step in flight per lane, refilling toward `laneLookahead` then parking. Its gate is the full fillQueue stop-set (`isDone || abortReason || capturedErrors.length`) — a lane step IS a pull, so no new step may start after close, a pending abort, or a fail-eventually capture (work already in flight still drains; `ordered: true` never steps a generator again after a capture and eager must match).
- **`nextValueEager()`** — the consumer: an **iterative** loop (`continue`, never self-recursion — thousands of settled phantom/done heads walked with no `await` crashed the stack at bufferSize ≈6000, the fillQueue bug class on the consumer side). Delivers `head.buffer` values in source order; only ever races the head's `pending` against the same fresh per-pull park / `ABORT_SENTINEL` machinery as `nextValue` (memory + exactly-once contracts identical). During a capture drain, a head with nothing buffered and nothing in flight is forfeited — its never-started values are cut off exactly like `ordered: true`'s. Errors surface in source order via `handleStreamError` at the head.
- **`classifyStep()`** — the eager twin of the fillOneSlot classification ladders (same WeakSet envelope brand; the two hot arms carry KEEP IN SYNC comments, and classifyStep must track them). `doCleanup` additionally `.return()`s live `lane.iterator`s (Set-deduped with everything else) and splices `lanes`.

### Iterator chaining via `currentStep`

`next()` chains each call's promise via `currentStep.then(nextValue, nextValue)` (both fulfilled and rejected handlers are `nextValue`) so that one rejection doesn't poison every subsequent call — the next call still re-enters `nextValue` (or `nextValueEager`), which then observes the post-rejection state machine (most often returning `{done:true}`).
